...
private:
	...
	int			mGold;					// An int value that describes the amount of gold the player has
	Weapon      mWeapon;				// The player's weapon, A weapon value describes the name of a weapon and its range of damage
	Spell		mSpell;

	int			mCharacterNum;			// An int value that stores the users selection for the character they choose after choosing their class/race
	
	int			mNumofMagicPoints;		// An int value that describes how many magic points the player currently has
	int			mMaxNumOfMagicPoints;	// An int value that describes the max magic points the player can have
};
